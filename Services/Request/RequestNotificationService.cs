...
namespace EmployeeApi.Services.Request;
...
public class RequestNotificationService : IRequestNotificationService
{
    ...
    public async Task SendApprovalEmailAsync(EmployeeApi.Models.Request request, string? comment)
    {
        try
        {
            ...
            {
                _logger.LogWarning("Cannot send approval email: requester not found or email missing for request {RequestId}", request.Id);
                return;
            }
            ...
            _logger.LogInformation(
                "Published approval email event for request {RequestId} to {Email}",
                request.Id, requester.Email);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
    ...
    public async Task SendRejectionEmailAsync(EmployeeApi.Models.Request request, string reason)
    {
        try
        {
            var requestTypeCode = request.RequestTypeLookup?.Code?.ToUpper() ?? "";
            ...
            if (requester == null || string.IsNullOrWhiteSpace(requester.Email))
            {
                _logger.LogWarning("Cannot send rejection email: requester not found or email missing for request {RequestId}", request.Id);
                return;
            }
            ...
            await _messageProducer.PublishMessage(emailEvent, SEND_EMAIL_QUEUE);
            _logger.LogInformation(
                "Published rejection email event for request {RequestId} to {Email}",
                request.Id, requester.Email);
        }
        catch (Exception ex)
        {
